    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage your product catalogue ({pagination.total} total)</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={openNew}
            className="inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2 font-semibold text-primary shadow-sm"
          >
      ...
      <div className="flex flex-wrap gap-4">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 bg-white py-2 pl-10 pr-4 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
          />
        </div>
        <select
          value={category}
                    ...
                    <tr key={p._id} className="border-t border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          <ActionBtn title="Edit" onClick={() => openEdit(p)}>
                            <Pencil className="h-4 w-4" />
                          </ActionBtn>
                          <ActionBtn
                            title="Quick edit name & price"
                            onClick={() => setQuickEditRow(p._id)}
                          >
                            Quick
                          </ActionBtn>
                          <ActionBtn title="View" onClick={() => setViewProduct(p)}>
...
function ActionBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`rounded p-1.5 hover:bg-slate-100 disabled:opacity-50 ${className}`}
    >
      {children}
    </button>
  );
